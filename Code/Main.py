...
def Vanderbilt_two_vehicle_ACC():
    ...
    analyze_statistics(uniform_format_path, step2_stat_result_path)
...
def CATS_UW():
    merge_data_list = []
    for i in range(1, 6):
        original_data_path = f'./Dataset/CATS/data/UW/Test{i}.csv'
        uniform_format_path = f'./Dataset/CATS/output/step1_UW_test{i}.csv'
        CATSUW_convert_format(original_data_path, uniform_format_path)
        merge_data_list.append(uniform_format_path)

    merge_data_path = f'./Dataset/CATS/output/step1_UW_merge.csv'
    merge_data(merge_data_list, merge_data_path)
    step1_stat_result_path = './Dataset/CATS/output/step1_analysis_UW'
    analyze_statistics(merge_data_path, step1_stat_result_path)

    clean_data = fill_and_clean(merge_data_path, 10, [None, 10, None, 10, 10, None, None],
                                1e10, -1e10, 1e10, -1e10,
                                1e10, -1e10, 1e10, -1e10, 1e10, -1e10)
    step2_data_path = f'./Dataset/CATS/output/step2_UW.csv'
    revise_traj_id(clean_data, step2_data_path, 0.1, 70, 0, 0)
    step2_stat_result_path = './Dataset/CATS/output/step2_analysis_UW'
    analyze_statistics(step2_data_path, step2_stat_result_path)
    ...
    clean_data = fill_and_clean(step2_data_path, 10, None,
                                120, 1e-5, 1e10, 0.1,
                                1e10, 0.1, 5, -5, 5, -5)
    step3_data_path = f'./Dataset/CATS/output/step3_UW.csv'
    revise_traj_id(clean_data, step3_data_path, 0.1, 70, 0, 0)
    step3_stat_result_path = './Dataset/CATS/output/step3_analysis_UW'
    analyze_statistics(step3_data_path, step3_stat_result_path)

    # Analysis
    performance_result_path = './Dataset/CATS/output/performance_metrics_UW.csv'
    analyze_AV_performance(step3_data_path, performance_result_path)
    scatter_plot_path = './Dataset/CATS/output/scatter_UW'
    draw_scatter(step3_data_path, scatter_plot_path)

    calibration_result_path = './Dataset/CATS/output/calibration_UW.csv'
    linear_regression = CFModelRegress(step3_data_path, 0.1)
    linear_regression.main(calibration_result_path, 'linear')
...
if __name__ == "__main__":
    ...
    paths = [
        ...
        './Dataset/CATS/output/performance_metrics_UW.csv',
    ...
    dataset_labels = ["Vanderbilt ACC", "CATS ACC", "CATS Platoon", "CATS UW",
    ...
    draw_2D_perfromance_metrics(paths[0:4], output_path + '1_', dataset_labels[0:4])
    draw_2D_perfromance_metrics(paths[4:8], output_path + '2_', dataset_labels[4:8])
    draw_2D_perfromance_metrics(paths[8:13], output_path + '3_', dataset_labels[8:13])

    # Compile statistics summary and draw distribution.
    output_path = './Dataset/statistics_summary_'
    merge_statistics_results(output_path)
    output_path = './Dataset/step3_distribution.png'
    ...
    output_path = f'./Dataset/labels_statistics_'
    dataset_labels = ["Vanderbilt ACC", "CATS ACC", "CATS Platoon", "CATS UW", "OpenACC Casale", "OpenACC Vicolungo",
                      "OpenACC ASta", "OpenACC ZalaZone", "Ohio Single", "Ohio Two", "Waymo Perception",
                      "Waymo Motion",
                      "Argoverse2"]
    draw_2D_labels_statistics(paths[0:4], output_path + '1_', dataset_labels[0:4])
    draw_2D_labels_statistics(paths[4:8], output_path + '2_', dataset_labels[4:8])
    draw_2D_labels_statistics(paths[8:13], output_path + '3_', dataset_labels[8:13])

    # Analyze label statistics and calculate correlation.
    output_path = './Dataset/correlation.csv'

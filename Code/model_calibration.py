...
class MyProblem(ea.Problem):
    ...
    def aimFunc(self, pop):
        ...
        for parameters in x:
            ...
            self.df['a_hat'] = self.df.apply(lambda row: IDM(arg, row['Space_Gap'],
                                                             row['Speed_FAV'],
...
class CFModelRegress:
    ...
    def addressOneTra(self, oneTraj):
        ...
        for t in range(delay_steps, oneTraj.shape[0]):
            xData.append([oneTraj['Space_Gap'].iloc[t - delay_steps],
    ...
    def IDM_regression(self, veh, xData, yData):
        ...
        df.columns = ['Space_Gap', 'Speed_FAV', 'Speed_Diff', 'Acc_FAV']
        ...
        df['a_hat'] = df.apply(lambda row: IDM(arg, row['Space_Gap'],
                                               row['Speed_FAV'],
                                               -row['Speed_Diff']), axis=1)
